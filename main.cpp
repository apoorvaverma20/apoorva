...
int moreIcon, startAt,  deviceToControl = -1, crossonTouch; //newgeneric
...
Widget AdvanceButton(type.ColoredButton, 15, 190, 70, 30, 0, RGB(204, 0, 0), 1, 15.0f, 65535, L"Advanced");
 ...
 //printf("String displaying %s",opt);
...
Widget basicGetButton(type.Button, 85, 115, 70, 30, 0, lan.get("get", "common")), basicGetTextBox(type.TextBox, 165, 115, 70, 30, 0, L""),
...
Widget Mybutton(type.Button,50 ,50, 90, 50, 0, L"CLICK HERE");
...
void display(void)
{
 ...
 drawText(L"Hello",50,170,250,1,0,16.0f,65535,1);
 Mybutton.draw();                                    //this will display the button on screen
 Mybutton.onTouch=&Mybutton_onTouch;
 Image=pngImg.loadPNG("/almond/guide_mac.png");
 pngImg.drawPNG(150, 50, 0, BACKGROUND_LAYER, Image);
...
void Mybutton_onTouch(void)
{
  ...
  //drawText(L"HELLO EVERYONE",50,150,150,1,0,20.0f,65535,1);  //when i am touching near Click in button it is displaying text?
}
...
void init(void) {
   ...
   // kb10->keyboardReturn = &NumericKbReturn;
...
connected_home->getTouch(x, y);
       ...
       /* if (x > 275 && x < 275 + 40 && y > 5 && y < 5 + 40) {
            ...
            FILE *fp;
            if (fp = fopenlcd_clear(BACKGROUND_LAYER);("/almond/email", "r")) {
                fgets(email, 180, fp);
                strcpy(data + 2, email);
                fclose(fp);
            }
